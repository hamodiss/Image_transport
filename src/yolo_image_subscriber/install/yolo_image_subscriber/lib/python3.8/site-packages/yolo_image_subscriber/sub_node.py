...
class ImageSubscriber(Node):
    ...
    def send_warning(self, label):
        """Simulate sending a warning signal by calling an external script."""
        try:
            # Define the absolute path to the warning script
            script_path = "/root/Image_transport/src/yolo_image_subscriber/yolo_image_subscriber/warning_script.py"
            
            # Call the external script for the warning
            subprocess.run(["python3", script_path, label], check=True)
            
            # Log successful warning message
            self.get_logger().info(f"Warning sent: Object '{label}' detected in the restricted zone!")
        except subprocess.CalledProcessError as e:
            # Log the error if the subprocess fails
            self.get_logger().error(f"Failed to send warning: {e}")
                ...
                if label in restricted_objects:
                    ...
                    if restricted_zone:
                        ...
                        if ((x1 >= x_min and x1 <= x_max and y1 >= y_min and y1 <= y_max) or
                            ...
                            self.get_logger().info(f"Object: {label} is partially or fully IN the restricted zone")
                            self.send_warning(label)
                        else:
